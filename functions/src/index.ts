...
export const processVideo = onCall(
  ...
  async (request) => {
    ...
    let totalOcrOutputTokens = 0;
    let totalTtsCharCount = 0;

    const addUsage = (usage: any) => {
      if (usage) {
        totalOcrInputTokens += usage.promptTokenCount || 0;
        totalOcrOutputTokens += usage.candidatesTokenCount || 0;
      }
    };
...
5. FORMATTING:
   ...
   - Mark chapter starts or major section titles with [CHAPTER: Chapter Title].`,
        ...
        addUsage((extractionResponse as any).usageMetadata);
...
${preCleanedText}`,
      ...
      addUsage((cleaningResponse as any).usageMetadata);
...
${rawText.substring(0, 3000)}`,
        ...
        addUsage((metadataResponse as any).usageMetadata);
      ...
      try {
        const watermarkText = `This is a personal reading of the book ${detectedTitle} by ${detectedAuthor}, generated by PaperEcho for the user with email address ${request.auth.token.email || 'anonymous'}. Distribution or sharing of this file is strictly prohibited.`;
        totalTtsCharCount += watermarkText.length;
      ...
      for (let i = 0; i < textChunks.length; i++) {
        totalTtsCharCount += textChunks[i].length;
      ...
      const ocrInputCost = totalOcrInputTokens * (0.075 / 1000000);
      const ocrOutputCost = totalOcrOutputTokens * (0.30 / 1000000);
      const ttsCost = totalTtsCharCount * 0.000005;
      const estimatedCostUsd = Number((ocrInputCost + ocrOutputCost + ttsCost).toFixed(4));

      const tokenUsage = {
        ocrInputTokens: totalOcrInputTokens,
        ocrOutputTokens: totalOcrOutputTokens,
        ttsCharCount: totalTtsCharCount,
        estimatedCostUsd,
      };
